...
export default function LiveChat({ streamId, user, className = "" }) {
  ...
  return (
    <div className={`flex flex-col bg-victory-card border border-victory-border rounded-xl overflow-hidden ${className}`}>
        ...
        {messages.map((msg, i) => (
          <div key={msg.message_id || i} className="flex gap-2 items-start">
            ...
            <div className="min-w-0">
              <span className="text-victory-lime text-xs font-semibold mr-1.5">{msg.user_name}</span>
      ...
      {/* Input */}
      <div className="flex gap-2 p-2 border-t border-victory-border">
        <input
          value={input}
        ...
        <button
          ...
          className="w-9 h-9 flex items-center justify-center rounded-lg bg-victory-lime text-victory-bg disabled:opacity-40 transition-opacity"
        >
