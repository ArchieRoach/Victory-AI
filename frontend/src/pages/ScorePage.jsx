...
const DIMENSION_GROUPS = {
  "Offensive Technique": [
    ...
    "Combination Flow", "Punch Balance", "Punch Accuracy",
  ],
  "Defensive Technique": [
    "Guard Position", "Head Movement", "Slip", "Roll", "Parry", "Body Movement",
  ],
...
const AnalysisOverlay = ({ progress, stepIndex }) => {
  ...
  const steps = [
    t("score.analysis.step0"),
    t("score.analysis.step1"),
    t("score.analysis.step2"),
    t("score.analysis.step3"),
    t("score.analysis.step4"),
    t("score.analysis.step5"),
  ];
  return (
    <div className="fixed inset-0 z-50 bg-victory-bg flex flex-col items-center justify-center p-8">
      ...
      <h2 className="text-2xl font-heading font-extrabold text-victory-text mb-2 text-center">
        {t("score.analysis.title")}
      </h2>
      <p className="text-victory-muted text-sm mb-8 text-center h-5 animate-pulse">
...
export default function ScorePage() {
  ...
  const saveDraft = useCallback(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ scores, videoUrl, sessionNotes, sessionDate }));
  ...
  const handleScoreChange = (dimension, value) => setScores((prev) => ({ ...prev, [dimension]: value }));
  const handleSkip = (dimension) => setScores((prev) => { const n = { ...prev }; delete n[dimension]; return n; });
  ...
  const handleSubmit = async () => {
    ...
    if (scoredCount < 5) {
      toast.error(t("score.scoreAtLeast", { count: scoredCount }));
      return;
    }
    ...
    analysisIntervalRef.current = setInterval(() => {
      ...
      setAnalysisStep(Math.floor((pct / 90) * (STEP_COUNT - 1)));
    }, interval);

    try {
      const dimensionScores = Object.values(DIMENSION_GROUPS).flat().map((dim) => ({
        dimension_name: dim,
  ...
  return (
    <div className="min-h-screen bg-victory-bg pb-nav" data-testid="score-page">
      {loading && <AnalysisOverlay progress={analysisProgress} stepIndex={analysisStep} />}
      ...
      <header className="sticky top-0 z-40 bg-victory-bg/95 backdrop-blur-sm border-b border-victory-border p-4">
        ...
        <p className="text-victory-muted text-sm mb-3">{t("score.subtitle")}</p>
        <div className="flex items-center gap-3">
          <Progress value={getProgress()} className="flex-1 h-2" />
          <span className="text-victory-muted text-sm font-mono">
            {getScoredCount()} {t("score.of")} {getTotalDimensions()}
          </span>
        </div>
      </header>
      ...
      <main className="p-4 space-y-6">
        <section className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={t("score.videoPlaceholder")}
          ...
          <div>
            <label className="victory-label">{t("score.dateLabel")}</label>
            <input
              type="date"
              value={sessionDate}
              ...
              >
                <div className="flex items-center gap-3">
                  <h2 className="font-heading font-bold text-victory-text">
                    {t(`score.groups.${groupName}`, groupName)}
                  </h2>
                  <span className="text-victory-muted text-sm">
                    {dimensions.filter((d) => scores[d] !== undefined).length}/{dimensions.length}
                  </span>
                </div>
        ...
        {getScoredCount() < 5 && getScoredCount() > 0 && (
          <p className="text-victory-orange text-sm text-center">
            {t("score.scoreAtLeast", { count: getScoredCount() })}
          </p>
        )}
      </main>
